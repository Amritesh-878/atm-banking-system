...
const BalanceScreen = () => {
  ...
  const filteredDbTransactions = Array.isArray(allDbTransactions)
    ? allDbTransactions.filter((txn) => {
        ...
        const userId =
          currentUser && currentUser.customerNumber
            ? String(currentUser.customerNumber).trim()
            : "";
        const userAltId =
          currentUser && currentUser.id ? String(currentUser.id).trim() : "";
      ...
      <div className="max-w-2xl mx-auto p-6">
        ...
        <div className="bg-gray-50 rounded-xl shadow-lg p-6 mt-8">
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {Array.isArray(filteredDbTransactions) &&
              filteredDbTransactions.length > 0 ? (
                filteredDbTransactions.map((txn, idx) => (
                  <tr key={idx} className="border-t">
                    <td className="py-2 px-2">
                      {txn.account || txn.accountType || "-"}
                    </td>
                    ...
                    <td className="py-2 px-2 text-right">
                      {txn.balance && !isNaN(Number(txn.balance))
